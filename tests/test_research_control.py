...
def preflight_args(tmp_path: Path) -> argparse.Namespace:
...
def test_deerflow_preflight_pass_advances_state(tmp_path: Path) -> None:
    ...
    with patch("scripts.research_control.run_command", return_value=0):
        assert research_control.deerflow_preflight(preflight_args(tmp_path)) == 0
    state = read_state(tmp_path)
    assert state["state"] == "DEERFLOW_READY"
    assert state["deerflow_preflight_inputs"]["deployment_manifest"]
...
def test_deerflow_preflight_failure_blocks_external(tmp_path: Path) -> None:
    ...
    with patch("scripts.research_control.run_command", return_value=1):
        assert research_control.deerflow_preflight(preflight_args(tmp_path)) == 1
    assert read_state(tmp_path)["state"] == "BLOCKED_EXTERNAL"


def test_deerflow_preflight_missing_report_blocks_external(tmp_path: Path) -> None:
    research_control.write_json(
        tmp_path / "run_state.json",
        {"state": "PLANNED", "history": []},
    )
    with patch("scripts.research_control.run_command", return_value=2):
        assert research_control.deerflow_preflight(preflight_args(tmp_path)) == 1
    state = read_state(tmp_path)
    assert state["state"] == "BLOCKED_EXTERNAL"
    assert "no valid report" in state["history"][-1]["reason"]
...
def test_sensitive_release_without_human_approval_stops(tmp_path: Path) -> None:
    ...
    research_control.write_json(
        tmp_path / "run_state.json",
        {
            "state": "REVIEWED",
            "history": [],
            "deerflow_preflight_inputs": {
                "deployment_manifest": str(deployment),
                "extensions_config": str(extensions),
                "config": None,
                "skip_live_agent_inventory": False,
            },
        },
    ...
    for filename in (
        ...
        "deerflow_release_preflight_report.json",
